...
class TestACPCapabilityProvider:
    ...
    def full_capabilities(self):
        """Client capabilities with all features enabled."""
    ...
    @pytest.fixture
    def terminal_only_capabilities(self):
        """Client capabilities with only terminal enabled."""
        return ClientCapabilities(
            terminal=True,
        )

    @pytest.fixture
    def fs_only_capabilities(self):
        """Client capabilities with only filesystem enabled."""
        return ClientCapabilities(
            fs=FileSystemCapability(read_text_file=True, write_text_file=True),
        )
    ...
    async def test_provider_creates_all_tools_with_full_capabilities(
        ...
        """Test provider creates all tools when client has full capabilities."""
        provider = ACPCapabilityResourceProvider(
            agent=acp_agent,
            session_id="test_session",
            client_capabilities=full_capabilities,
        )

        tools = await provider.get_tools()
        tool_names = {tool.name for tool in tools}

        # Should have all terminal tools
        ...
        expected_fs_tools = {"read_text_file", "write_text_file"}
    ...
    async def test_provider_creates_only_terminal_tools_with_terminal_capabilities(
        ...
        """Test provider creates only terminal tools when client only has terminal."""
        provider = ACPCapabilityResourceProvider(
            agent=acp_agent,
            session_id="test_session",
            client_capabilities=terminal_only_capabilities,
        )
    ...
    async def test_provider_creates_only_fs_tools_with_fs_capabilities(
        ...
        """Test provider creates only filesystem tools when client only has filesystem."""
        provider = ACPCapabilityResourceProvider(
            agent=acp_agent,
            session_id="test_session",
            client_capabilities=fs_only_capabilities,
        )
        ...
        # Should NOT have terminal tools
        terminal_tools = {
            "run_command",
            "get_command_output",
            "create_terminal",
    ...
    async def test_provider_creates_no_tools_with_no_capabilities(
        ...
        """Test provider creates no tools when client has no capabilities."""
        provider = ACPCapabilityResourceProvider(
            agent=acp_agent,
            session_id="test_session",
